...
namespace GoalsPlanningSystem.Api.Simulation;

public abstract record BuildResult;
public sealed record BuildNotFound : BuildResult;
public sealed record BuildProblem(string Message) : BuildResult;
public sealed record BuildSuccess(Plan Plan, SimulationRequest Request) : BuildResult;

/// <summary>
/// Shared between ProjectionsController and InsightsController: both need the exact same plan ->
/// SimulationRequest translation, and InsightsController additionally needs the baseline request as a
/// starting point to build "what-if" variants from (see SimulationRequest's `with` expressions).
/// </summary>
public class SimulationRequestBuilder(GoalsPlanningSystemDbContext db)
{
    private const decimal DefaultNpsAnnuitizationPct = 40m;
    private const decimal DefaultAssumedAnnuityRatePct = 6m;

    public async Task<BuildResult> BuildAsync(int planId, int userId)
    {
        var plan = await db.Plans.AsNoTracking().Include(p => p.Clients).FirstOrDefaultAsync(p => p.Id == planId && p.UserId == userId);
        if (plan is null) return new BuildNotFound();

        if (plan.Clients.Count == 0)
        {
            return new BuildProblem("Plan has no clients to project.");
        }
        ...
        if (accounts.Count == 0)
        {
            return new BuildProblem("Plan has no accounts to project.");
        }
        ...
        var request = new SimulationRequest(
            ...
            Expenses: expenses.Select(e => new ExpenseInput(e.AnnualAmount, e.StartYear, e.EndYear, e.GrowthRateOverridePct ?? plan.InflationRatePct, e.Category)).ToList(),
        ...
        return new BuildSuccess(plan, request);
    }
