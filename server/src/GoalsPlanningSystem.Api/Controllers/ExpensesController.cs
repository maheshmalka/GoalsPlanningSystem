...
[ApiController]
[Route("api/clients/{clientId:int}/expenses")]
public class ExpensesController(GoalsPlanningSystemDbContext db) : ControllerBase
{
    private static ExpenseDto ToDto(Expense e) => new(e.Id, e.ClientId, e.Name, e.Category, e.AnnualAmount, e.StartYear, e.EndYear, e.GrowthRateOverridePct);

    [HttpGet]
    public async Task<ActionResult<List<ExpenseDto>>> GetAll(int clientId)
    {
    ...
    [HttpPost]
    public async Task<ActionResult<ExpenseDto>> Create(int clientId, ExpenseUpsertDto dto)
    {
        if (!await db.Clients.AnyAsync(c => c.Id == clientId)) return NotFound();

        var expense = new Expense
        {
            ClientId = clientId, Name = dto.Name, Category = dto.Category, AnnualAmount = dto.AnnualAmount,
            StartYear = dto.StartYear, EndYear = dto.EndYear, GrowthRateOverridePct = dto.GrowthRateOverridePct
        };
        db.Expenses.Add(expense);
        await db.SaveChangesAsync();
        return CreatedAtAction(nameof(GetAll), new { clientId }, ToDto(expense));
    }

    [HttpPut("{id:int}")]
    public async Task<ActionResult<ExpenseDto>> Update(int clientId, int id, ExpenseUpsertDto dto)
    {
        var expense = await db.Expenses.FirstOrDefaultAsync(e => e.Id == id && e.ClientId == clientId);
